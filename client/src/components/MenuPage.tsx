...
export default function MenuPage(): JSX.Element {
  ...
  const { addToCart, cart } = useContext(CartContext) as any;
  ...
  const filters = [
    {
      key: "all",
      label: "All Items",
      icon: <IoFastFood className="text-orange-500" />,
    ...
    },
    {
      ...
      count: menuItems.filter((item) => item.rating >= 4.0).length,
    },
  ];
  ...
  const totalItems =
    cart?.reduce((total: number, item: any) => total + (item.qty || 1), 0) || 0;
  const cartTotal =
    cart?.reduce(
      (total: number, item: any) => total + item.price * (item.qty || 1),
      0,
    ) || 0;

  const handleAddToCart = (item: any) => {
    addToCart({ ...item, restaurantId: id });
    setQuantity((prev) => ({ ...prev, [item.id]: (prev[item.id] || 0) + 1 }));
  };
  ...
  const FilterButton = ({
    ...
    <button
      onClick={() => {
        if (filter.key === "all") setActiveFilters([]);
        else {
      ...
      className={`flex items-center gap-1 px-3 py-2 rounded-lg border transition-all duration-300 font-semibold shrink-0 text-sm ${activeFilters.includes(filter.key) ? "bg-orange-500 border-orange-500 text-white" : "bg-white border-gray-200 text-gray-700 hover:border-orange-300"} ${mobile ? "text-xs px-2 py-1.5" : ""}`}
    >
      <span className="text-sm">{filter.icon}</span>
      <span className="whitespace-nowrap">{filter.label}</span>
      {filter.count > 0 && (
        <span
          className={`px-1.5 py-0.5 rounded text-xs ${activeFilters.includes(filter.key) ? "bg-orange-400 text-white" : "bg-orange-100 text-orange-600"}`}
        >
          {filter.count}
        </span>
      )}
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {sortedItems.length}{" "}
              {search ? "Search Results" : "Dishes Available"}
            </h2>
          ...
          {sortedItems.map((item: any) => (
            ...
            >
              <div className="relative overflow-hidden">
                ...
                <div
                  className={`absolute top-3 right-3 w-6 h-6 rounded-full border-2 ${item.veg ? "border-green-500 bg-green-500" : "border-red-500 bg-red-500"}`}
                >
                  <div
                    className={`w-2 h-2 rounded-full bg-white m-1 ${item.veg ? "bg-green-500" : "bg-red-500"}`}
                  ></div>
                </div>
              </div>
