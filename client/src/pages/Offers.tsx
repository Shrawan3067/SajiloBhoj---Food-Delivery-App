...
type Offer = any; // permissive for incremental migration

export default function Offers(): JSX.Element {
  const [offerCategories, setOfferCategories] = useState<any[]>([
    {
      id: 1,
      title: "All Offers",
  ...
  const filteredOffers =
    activeFilter === "all"
      ? allOffers
      : allOffers.filter((o) => o.category === activeFilter);

  useEffect(() => {
    const updated = offerCategories.map((cat) => {
      if (cat.filter === "all") return { ...cat, count: allOffers.length };
      const count = allOffers.filter(
        (offer) => offer.category === cat.filter,
      ).length;
      return { ...cat, count };
    });
    setOfferCategories(updated);
  ...
  const handleCategoryClick = (filter: string) => {
    ...
    setOfferCategories((prev) =>
      prev.map((c: any) => ({ ...c, active: c.filter === filter })),
    );
  };
  ...
  const getActiveOffersCount = () =>
    activeFilter === "all" ? allOffers.length : filteredOffers.length;
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4">
              <p className="text-white font-semibold">
                <span className="text-2xl">{getActiveOffersCount()}</span>{" "}
                Active Offers
                {activeFilter !== "all" && (
                  <span className="text-sm block mt-1">
                    in{" "}
                    {
                      offerCategories.find((c) => c.filter === activeFilter)
                        ?.title
                    }
                  </span>
                )}
          ...
          <div className="flex overflow-x-auto gap-3 pb-2 scrollbar-hide">
            {offerCategories.map((category) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-3 rounded-xl whitespace-nowrap transition-all ${category.active ? "bg-orange-500 text-white shadow-md" : "bg-white text-gray-700 hover:bg-gray-100"}`}
              >
                {category.icon}
                <span className="font-medium">{category.title}</span>
                <span
                  className={`ml-2 px-2 py-1 rounded-full text-xs ${category.active ? "bg-white/30" : "bg-gray-100 text-gray-600"}`}
                >
                  {category.count}
                </span>
              </button>
        ...
        <div className="mb-12">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold">
              {activeFilter === "all"
                ? "All Active Offers"
                : `${offerCategories.find((c) => c.filter === activeFilter)?.title}`}
            </h2>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-md">
          ...
          <ul className="space-y-2 text-gray-600 text-sm">
            ...
            <li className="flex items-start gap-2">
              <span className="text-orange-500 mt-1">•</span>
              <span>
                BiteXpress reserves the right to modify or cancel offers
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-orange-500 mt-1">•</span>
              <span>
                In case of any dispute, BiteXpress's decision will be final
              </span>
            </li>
          </ul>
        </div>
